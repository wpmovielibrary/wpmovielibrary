...
if ( 'input' == $_type ) : ?>
	...
	<input type="text" id="<?php echo $_id ?>" name="<?php echo $_id ?>" value="<?php echo $_value ?>" size="40" maxlength="32" />
	<p class="description"><?php echo $field['description'] ?></p>

<?php elseif ( 'toggle' == $_type ) : ?>

	<label><input type="radio" id="<?php echo $_id ?>" name="<?php echo $_id ?>" value="1"<?php checked( $_value, 1 ); ?>/> <?php echo $_title ?></label>
	<label><input type="radio" id="<?php echo $_id ?>" name="<?php echo $_id ?>" value="0"<?php checked( $_value, 0 ); ?>/> <?php _e( 'Don&rsquo;t', 'wpml' ); ?></label>
	<p class="description"><?php echo $field['description'] ?></p>

<?php elseif ( 'select' == $_type || 'multiple' == $_type ) : ?>

	<select id="<?php echo $_id ?>" name="<?php echo $_id ?>"<?php if ( 'multiple' == $_type ) echo ' multiple="multiple"' ?>>
...
<?php elseif ( 'sorted' == $_type ) : ?>
								...
								<select id="default_movie_meta" name="tmdb_data[wpml][default_movie_meta][]" class="hide-if-js" style="min-height:<?php echo count( $items ) ?>em;min-width:16em;" multiple>
									<?php echo $options ?>
								</select>
