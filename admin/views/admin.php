...
<?php endforeach; ?>
						...
						<tr valign="top">
							...
							<td>
								<select id="default_movie_details" name="tmdb_data[wpml][default_movie_details][]" style="min-height:<?php echo round( count( $this->default_post_detail ) * 1.4, 1 ) ?>em;min-width:12em;" multiple>
<?php
foreach ( $this->wpml->wpml_movie_details as $slug => $detail ) :
	$check = in_array( $slug, $this->wpml_o('wpml-settings-default_movie_details' ) ) || in_array( $detail['title'], $this->wpml_o('wpml-settings-default_movie_details' ) );
?>
