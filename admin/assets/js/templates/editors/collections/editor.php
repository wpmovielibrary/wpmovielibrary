<?php
/**
 * Collections Editor Main Template
 *
 * @since 3.0.0
 */
?>

		<div data-headbox="{{ data.id }}" data-theme="default" class="wpmoly term-headbox collection-headbox theme-default">
			...
			<div class="headbox-content clearfix">
				<div class="headbox-titles">
					<div class="headbox-title">
						<div class="term-title collection-title">{{ data.name }}</div>
					</div>
					<div class="headbox-subtitle">
						<div class="term-count collection-count">{{{ wpmoly._n( wpmolyEditorL10n.n_total_post, data.count ) }}}</div>
					</div>
				</div>
				<div class="headbox-metadata">
					<div class="headbox-description">
						<div class="term-description collection-description">{{ data.description || s.sprintf( wpmolyEditorL10n.no_description, data.name ) }}</div>
					</div>
				</div>
			</div>
		</div>
