<?php
/**
 * Actor Browser Discover Block Template
 *
 * @since 3.0.0
 *
 * @uses $id
...
?>
					...
					<div id="<?php echo esc_attr( $id ); ?>-block" data-controller="<?php echo esc_attr( $controller ); ?>" class="<?php echo esc_attr( $class ); ?>">
						...
						<div class="block-content">
							<p><?php printf( _n( 'You have a total of <a href="%s">%s actor</a>.', 'You have a total of <a href="%s">%s actors</a>.', $total, 'wpmovielibrary' ), esc_url( $edit ), '<strong>' . $total . '</strong>' ); ?></p>
						</div>
					</div>
