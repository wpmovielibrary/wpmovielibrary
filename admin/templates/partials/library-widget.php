	...
	<div class="wpmovielibrary-widget-content">
		...
		<ul>
			<li>
				...
				@else
					{!! esc_html__('No movies', 'wpmovielibrary') !!}
				@endif
			...
			<li>
				...
				@if ( 0 < $totals['drafts'] )
					{!! esc_html( sprintf( _n( '%s draft', '%s drafts', $totals['drafts'], 'wpmovielibrary' ), $totals['drafts'] ) ) !!}</a>
				@else
					{!! esc_html__('No drafts', 'wpmovielibrary') !!}
				@endif
		...
		<p>{!! sprintf(
				...
				'<a href="' . esc_url( admin_url( 'edit-tags.php?taxonomy=collection&post_type=movie' ) ) . '">' . sprintf( _n( '<strong>1</strong> collection', '<strong>%s</strong> collections', $totals['collections'], 'wpmovielibrary' ), $totals['collections'] ?? '0' ) . '</a>',
				'<a href="' . esc_url( admin_url( 'edit-tags.php?taxonomy=genre&post_type=movie' ) ) . '">' . sprintf( _n( '<strong>1</strong> genre', '<strong>%s</strong> genres', $totals['genres'], 'wpmovielibrary' ), $totals['genres'] ?? '0' ) . '</a>',
				'<a href="' . esc_url( admin_url( 'edit-tags.php?taxonomy=actor&post_type=movie' ) ) . '">' . sprintf( _n( '<strong>1</strong> actor', '<strong>%s</strong> actors', $totals['actors'], 'wpmovielibrary' ), $totals['actors'] ?? '0' ) . '</a>'
			) !!}</p>
	</div>
</div>