<?php
/**
 * Web interface for generating the WPMovieLibrary POT files
 *
 * @since 2.0
...
require 'makepot.php';
$makepot = new WPML_Makepot;

// Regeneration requested
if ( (isset($_GET) and ! empty( $_GET['generate']) || (isset($argc, $argv) && $argc==2 && $argv[1]=="generate")) ) {
	// Generate wpmovielibrary and wpmovielibrary-admin POT files
	$results = array();
...
// Load WPMovieLibrary POT-files info
$pot_files = array();
foreach( $makepot->projects as $name => $project ) {
...
if($is_shell) {
	printf("WPMovieLibrary %s POT Generator\n\n", $makepot->wpmovielibrary_version());
	if ( ! empty( $results ) ) {
	...
	echo "This tool will (re)generate and overwrite the following WPMovieLibrary POT-files:\n\n";
...
<head>
	...
	<meta charset="utf-8">
	<title>WPMovieLibrary POT Generator</title>
	<style>
...
<body>
	...
	<h1>WPMovieLibrary <?php echo $makepot->wpmovielibrary_version() ?> POT Generator</h1>
	...
	<p>This tool will (re)generate and overwrite the following WPMovieLibrary POT-files:</p>
