...
class WPML_Status_Widget extends WP_Widget {
	...
	public function __construct() {
		...
		parent::__construct(
			'wpml-status-widget',
			__( 'WPML Status', 'wpmovielibrary' ),
			array(
				'classname'	=>	'wpml-status-widget',
				'description'	=>	__( 'Display Movies from a specific status', 'wpmovielibrary' )
			)
		);
	}
		...
		if ( $status_only ) {
			...
			if ( ! empty( $status ) ) {
				...
				foreach ( $status as $slug => $status_title ) {
					...
					$items[] = array(
						'ID'          => $slug,
						'attr_title'  => sprintf( __( 'Permalink for &laquo; %s &raquo;', 'wpmovielibrary' ), $status_title ),
				...
				$attributes = array( 'items' => $items, 'description' => $description, 'default_option' => __( 'Select a status', 'wpmovielibrary' ), 'style' => $style );
	...
	public function form( $instance ) {
		...
		$instance['title']       = ( isset( $instance['title'] ) ? $instance['title'] : __( 'Movie by Status', 'wpmovielibrary' ) );
