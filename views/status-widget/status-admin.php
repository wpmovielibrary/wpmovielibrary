...
?>
	...
	<p>
			...
			<label for="<?php echo $widget->get_field_id( 'list' ); ?>"><?php _e( 'Display as dropdown', 'wpmovielibrary' ); ?></label><br />
	</p>
	...
	<div id="_movies_only_<?php echo $widget->get_field_id( 'status_only' ); ?>" class="<?php if ( 0 !== $status_only ) echo 'hide-if-js'; ?>">
		<p>
			<label for="<?php echo $widget->get_field_id( 'type' ); ?>"><h4 class="wpml-widget-title"><?php _e( 'Status', 'wpmovielibrary' ); ?></h4></label>
			...
			<?php foreach ( WPML_Settings::get_available_movie_status() as $slug => $title ) : ?>
				<option value="<?php echo $slug ?>" <?php selected( $type, $slug ) ?>><?php _e( $title, 'wpmovielibrary' ) ?></option>
			<?php endforeach; ?>
			</select>
		</p>
		<p>
			...
			<label for="<?php echo $widget->get_field_id( 'thumbnails' ); ?>"><?php _e( 'Show thumbnails', 'wpmovielibrary' ); ?></label>
		</p>
	</div>