...
<?php foreach ( $default_movie_status as $slug => $title ) : ?>
				<li><span class="dashicons dashicons-arrow-right-alt2"></span> <a href="#" onclick="wpmoly_edit_details.inline_edit( 'status', this ); return false;" data-status="<?php echo $slug ?>" data-status-title="<?php _e( $title, 'wpmovielibrary' ) ?>"><?php _e( $title, 'wpmovielibrary' ) ?></a></li>
<?php endforeach; ?>
			</ul>
		</div>
...
<?php foreach ( $default_movie_media as $slug => $title ) : ?>
				<li><span class="dashicons dashicons-arrow-right-alt2"></span> <a href="#" onclick="wpmoly_edit_details.inline_edit( 'media', this ); return false;" data-media="<?php echo $slug ?>" data-media-title="<?php _e( $title, 'wpmovielibrary' ) ?>"><?php _e( $title, 'wpmovielibrary' ) ?></a></li>
<?php endforeach; ?>
			</ul>
		</div>
