...
?>
	...
	<p>
		<label for="<?php echo $widget->get_field_id( 'meta' ); ?>"><strong class="wpmoly-widget-title"><?php _e( 'Meta', 'wpmovielibrary' ); ?></strong></label><br />
		<select id="<?php echo $widget->get_field_id( 'meta' ); ?>" name="<?php echo $widget->get_field_name( 'meta' ); ?>">
<?php
$supported = WPMOLY_Settings::get_supported_movie_meta();
foreach ( $supported as $slug => $s ) :
?>
			<option value="<?php echo $slug ?>" <?php selected( $slug, $meta ); ?>><?php _e( $s['title'], 'wpmovielibrary' ); ?></option>
