	...
	array(
		...
		'fields'  => array(
			...
			'backdrops-texts' => array(
				...
				'style'  => 'info',
				'title'  => __( 'Allowed tags', 'wpmovielibrary' ),
				'desc'   => sprintf( __( '<br />%s<br /><br />To limit the number of elements in list like genres or production companies, use the <code>{tag,number}</code> synthax. Example: <code>{production_companies,1}</code> will use the first company from list instead of the whole list.', 'wpmovielibrary' ), apply_filters( 'wpmoly/options/backdrops/allowed_tags', '<code>{tmdb_id}</code>, <code>{imdb_id}</code>, <code>{title}</code>, <code>{original_title}</code>, <code>{tagline}</code>, <code>{release_date}</code>, <code>{year}</code>, <code>{local_release_date}</code>, <code>{local_year}</code>, <code>{runtime}</code>, <code>{production_companies}</code>, <code>{production_countries}</code>, <code>{spoken_languages}</code>, <code>{genres}</code>, <code>{director}</code>, <code>{producer}</code>, <code>{photography}</code>, <code>{composer}</code>, <code>{author}</code>, <code>{writer}</code> and <code>{certification}</code>' ) )
			)
		),
	),
